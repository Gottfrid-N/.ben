            ...
            match self.current_char:
                case self.current_id_open:
                    ...
                    while self.current_char != self.current_id_close:
                        ...
                        self.advance()
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_ID, "\"" + self.buffer.__str__() + "\""))
                    self.buffer.clear()
                    self.current_id_open = self.current_id_close = None

                case metalbeard.tokens.TC_VARIABLE:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_VARIABLE, metalbeard.tokens.TV_SINGLE))
                    self.current_id_open = self.current_id_close = metalbeard.tokens.TC_VARIABLE
                    continue

                case metalbeard.tokens.TC_MACRO:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_MACRO, metalbeard.tokens.TV_SINGLE))
                    self.current_id_open = self.current_id_close = metalbeard.tokens.TC_MACRO
                    continue

                case metalbeard.tokens.TC_STRING:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_STRING, metalbeard.tokens.TV_SINGLE))
                    self.current_id_open = self.current_id_close = metalbeard.tokens.TC_STRING
                    continue

                case metalbeard.tokens.TC_SEPERATOR:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_SEPERATOR, metalbeard.tokens.TV_SINGLE))

                case metalbeard.tokens.TC_SCOPE_OPEN:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_SCOPE, metalbeard.tokens.TV_OPEN))
                case metalbeard.tokens.TC_SCOPE_CLOSE:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_SCOPE, metalbeard.tokens.TV_CLOSE))

                case metalbeard.tokens.TC_GROUP_OPEN:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_GROUP, metalbeard.tokens.TV_OPEN))
                case metalbeard.tokens.TC_GROUP_CLOSE:
                    output.append(metalbeard.tokens.Token(metalbeard.tokens.TT_GROUP, metalbeard.tokens.TV_CLOSE))
